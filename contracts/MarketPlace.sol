...
contract Marketplace is ERC1155, Ownable {
        ...
        external
    {
        ...
        nfts[_tokenId] = NFT(_tokenId, msg.sender, address(0) ,100,false , address(0),_aesKeyHash);
        
    }

    function buyNFT(uint256 _id, bytes32  rsaPublicKey, address paymentToken) public{
        require(IERC20(paymentToken).balanceOf((msg.sender))>=nfts[_id].price, "Not enough balance");
        require(nfts[_id].sold == false, "Already Sold");
        setBuyerPublicKey(_id,rsaPublicKey);
        nfts[_id].sold = true;
        nfts[_id].paymentToken = paymentToken;
        IERC20(paymentToken).transferFrom(msg.sender, address(this), nfts[_id].price);
        ...
        external
    {
        ...
        IERC20(nfts[_tokenId].paymentToken).transferFrom(address(this), nfts[_tokenId].seller, nfts[_tokenId].price);
    ...
    function rsaEncrypt(bytes memory _data, bytes32 _publicKey) public view returns (bytes memory) {
    ...
    function modexp(uint256 base, uint256 exponent, uint256 modulus) public view returns (uint256 result) {
        assembly {
